...
def main():
    ...
    for i, strategy_set in enumerate(strategy_sets):
        ...
        for j, (strategy_name, result) in enumerate(results.items()):
            ...
            ax[j][i].bar(mpl_date, roi, alpha=0.5, color=bar_color, width=27)
            ax[j][i].plot(mpl_date, roi)

            ax[j][i].legend([f'{strategy_name} ({duration.days} Days)'])

            ax[j][i].xaxis.set_major_locator(mdates.YearLocator())
